...
class QuanLyView:
    def __init__(self, ui_path, app_context, on_logout):
        ...
        self.window.setFixedSize(1520, 820)
        ...
        setup_table(self.tblHangDoiUuTien, ["Mã KH", "Tên khách", "Dịch vụ", "Ưu tiên", "Thời gian đến"])
        setup_table(self.tblHangDoiThuong, ["Mã KH", "Tên khách", "Dịch vụ", "Ưu tiên", "Thời gian đến"])
        setup_table(self.tblQuay, ["Quầy", "Mở/Đóng", "Trạng thái", "Khách đang phục vụ"])
        setup_table(self.tblBaoCao, ["Mã KH", "Tên khách", "Loại dịch vụ", "Thời gian chờ", "Thời gian phục vụ", "Trạng thái"])
    ...
    def lam_moi(self):
        ...
        self.lblCanhBao.setStyleSheet("color: #dc2626;" if tk["canh_bao"] else "color: #16a34a;")

        fill_table(self.tblHangDoiUuTien, self._rows_khach(self.he_thong.lay_danh_sach_hang_doi_uu_tien()))
        fill_table(self.tblHangDoiThuong, self._rows_khach(self.he_thong.lay_danh_sach_hang_doi_thuong()))
        fill_table(self.tblQuay, self._rows_quay())
        ...
        fill_table(self.tblBaoCao, self._rows_bao_cao(ds))
    ...
    def mo_quay(self):
        ...
        QMessageBox.information(self.window, "Thông báo", msg) if ok else QMessageBox.warning(self.window, "Thông báo", msg)
        self.lam_moi()
    ...
    def _rows_khach(self, ds):
        rows = []
        for k in ds:
            rows.append([k.ma_khach(), k.ten, k.loai_dich_vu, k.muc_do_uu_tien, k.thoi_gian_den.strftime("%H:%M:%S")])
        return rows

    def _rows_quay(self):
        rows = []
        for q in self.he_thong.lay_danh_sach_quay():
            khach = "-" if q.khach_dang_phuc_vu is None else q.khach_dang_phuc_vu.ma_khach()
            rows.append([f"Quầy {q.id_quay}", "Mở" if q.dang_mo else "Đóng", q.trang_thai, khach])
        return rows

    def _rows_bao_cao(self, ds):
        rows = []
        for k in ds:
            rows.append([k.ma_khach(), k.ten, k.loai_dich_vu, f"{k.tinh_thoi_gian_cho():.1f} phút", f"{k.tinh_thoi_gian_phuc_vu():.1f} phút", k.trang_thai])
        return rows
